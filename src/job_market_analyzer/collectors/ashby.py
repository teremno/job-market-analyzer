"""Ashby collector using the official credential-free job board API."""
...
ASHBY_SOURCE_PROVIDER = "ashby"
ASHBY_USER_AGENT = "job-market-analyzer/0.1"

# Curated public Ashby boards approved for the pilot (documented in
# docs/SOURCES.md). Each token is one company's public posting API.
ASHBY_BOARD_TOKENS: tuple[str, ...] = (
    "openai",
    "elevenlabs",
    "clickhouse",
    "cohere",
...
class AshbyFeedError(ValueError):
    """Raised when an Ashby board returns an unsupported jobs response."""


class AshbyAPIError(RuntimeError):
    """Raised when every configured Ashby board fails."""
...
async def collect_ashby_boards(
    *,
    base_url: str = ASHBY_BASE_URL,
    board_tokens: tuple[str, ...] = ASHBY_BOARD_TOKENS,
    timeout: float = 30.0,
    ...
    if not board_tokens:
        raise ValueError("At least one Ashby board token is required")
        ...
        for token in board_tokens:
            ...
            for item_index, item in enumerate(board_jobs):
                try:
                    ...
                    failures.append(
                        CollectionFailure(
                            source_provider=ASHBY_SOURCE_PROVIDER,
                            stage="collect",
                            message=str(exc),
    ...
    if boards_ok == 0:
        raise AshbyAPIError("Every configured Ashby board request failed")
...
class AshbyCollector:
    """Collector adapter collecting all curated Ashby boards in one run."""
    ...
    async def collect(self) -> CollectedJobs:
        ...
        return await collect_ashby_boards(**self._options)
...
def _parse_board(response: httpx.Response, token: str) -> list[object]:
    try:
        ...
        raise AshbyFeedError(f"Board '{token}' returned invalid JSON") from exc
    if not isinstance(feed, dict) or not isinstance(feed.get("jobs"), list):
        raise AshbyFeedError(f"Board '{token}' response must contain a jobs array")
    return feed["jobs"]
...
def _to_raw_job(
    ...
    if not isinstance(item, dict):
        raise TypeError("Ashby job entry must be a JSON object")
    external_id = _required_text(item.get("id"), "id")
    source_url = _optional_text(item.get("jobUrl"), "jobUrl")
    return RawJob(
        source_provider=ASHBY_SOURCE_PROVIDER,
...
def _required_text(value: object, field_name: str) -> str:
    if isinstance(value, bool) or not isinstance(value, (str, int)):
        raise TypeError(f"Ashby field '{field_name}' must be a string or integer")
    text = str(value).strip()
    if not text:
        raise ValueError(f"Ashby field '{field_name}' must not be blank")
    return text
...
def _optional_text(value: object, field_name: str) -> str | None:
    ...
    if not isinstance(value, str):
        raise TypeError(f"Ashby field '{field_name}' must be a string")
    text = value.strip()
    return text or None
...
def _require_aware_clock(value: datetime) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise ValueError("AshbyCollector clock must return an aware datetime")
