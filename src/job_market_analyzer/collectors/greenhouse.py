"""Greenhouse collector using the official credential-free job board API."""
...
GREENHOUSE_SOURCE_PROVIDER = "greenhouse"
GREENHOUSE_USER_AGENT = "job-market-analyzer/0.1"

# Curated remote-friendly company boards approved for the pilot (ADR-documented
# in docs/SOURCES.md). Each token is one public Greenhouse job board.
...
class GreenhouseFeedError(ValueError):
    """Raised when a Greenhouse board returns an unsupported jobs response."""


class GreenhouseAPIError(RuntimeError):
    """Raised when every configured Greenhouse board fails."""
...
async def collect_greenhouse_boards(
    *,
    base_url: str = GREENHOUSE_BASE_URL,
    board_tokens: tuple[str, ...] = GREENHOUSE_BOARD_TOKENS,
    timeout: float = 30.0,
    transport: httpx.AsyncBaseTransport | None = None,
    clock: Callable[[], datetime] = _utc_now,
) -> CollectedJobs:
    """Fetch one bounded response per board and report per-board failures.
    ...
    async with httpx.AsyncClient(
        ...
        for token in board_tokens:
            try:
                ...
                continue
            except GreenhouseFeedError as exc:
    ...
    if boards_ok == 0:
        raise GreenhouseAPIError(
            "Every configured Greenhouse board request failed"
        )
...
def run_greenhouse_collection(**kwargs: object) -> CollectedJobs:
    """Synchronous entry point matching the collector factory contract."""

    return asyncio.run(collect_greenhouse_boards(**kwargs))


class GreenhouseCollector:
    """Collector adapter collecting all curated Greenhouse boards in one run."""
    ...
    async def collect(self) -> CollectedJobs:
        ...
        return await collect_greenhouse_boards(**self._options)
...
def _parse_board(response: httpx.Response, token: str) -> list[object]:
    try:
        ...
        raise GreenhouseFeedError(
            f"Board '{token}' returned invalid JSON"
        ) from exc
    if not isinstance(feed, dict) or not isinstance(feed.get("jobs"), list):
        raise GreenhouseFeedError(
            f"Board '{token}' response must contain a jobs array"
        )
    return feed["jobs"]
...
def _to_raw_job(
    ...
    if not isinstance(item, dict):
        raise TypeError("Greenhouse job entry must be a JSON object")
    external_id = _required_text(item.get("id"), "id")
    source_url = _optional_text(item.get("absolute_url"), "absolute_url")
    return RawJob(
        source_provider=GREENHOUSE_SOURCE_PROVIDER,
...
def _required_text(value: object, field_name: str) -> str:
    if isinstance(value, bool) or not isinstance(value, (str, int)):
        raise TypeError(f"Greenhouse field '{field_name}' must be a string or integer")
    text = str(value).strip()
    if not text:
        raise ValueError(f"Greenhouse field '{field_name}' must not be blank")
    return text
...
def _optional_text(value: object, field_name: str) -> str | None:
    ...
    if not isinstance(value, str):
        raise TypeError(f"Greenhouse field '{field_name}' must be a string")
    text = value.strip()
    return text or None
...
def _require_aware_clock(value: datetime) -> None:
    if value.tzinfo is None or value.utcoffset() is None:
        raise ValueError("GreenhouseCollector clock must return an aware datetime")
