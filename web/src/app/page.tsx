...
export default async function OverviewPage() {
  ...
  return <>
      ...
      <section className="insight-grid">
        <article className="panel"><div className="panel-heading"><div><p className="eyebrow">Experience axis</p><h2>Seniority signals</h2></div><Link href="/jobs?seniority=senior">Filter jobs</Link></div><BarList kind="roles" items={data.top_seniority.map((s) => ({ code: s.term_code, name: s.term_name, count: s.posting_count }))} /></article>
        <article className="panel"><div className="panel-heading"><div><p className="eyebrow">Work arrangement</p><h2>Remote / hybrid / onsite</h2></div><Link href="/jobs?geography=arrangement_remote">Remote jobs</Link></div><BarList kind="roles" items={data.arrangement_counts.map((a) => ({ code: a.term_code, name: a.term_name, count: a.posting_count }))} /></article>
      </section>
      <section className="insight-grid">
