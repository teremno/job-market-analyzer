...
Current registrations are `skills/en` and `roles/en`; `--language uk` is deliberately
rejected before database creation and collection. Future Ukrainian support requires
real `skills/uk` and `roles/uk` implementations and registry entries, not a source
mapping or an English fallback.
