...
- **API container** â€” the read-only FastAPI service (`job-market-analyzer serve`)
  bound to `0.0.0.0` inside the container, published only to loopback on the
  host by default.
- **Web container** â€” the Next.js dashboard calling the API server-side by
  service name.
- **CI** â€” GitHub Actions running Ruff, the full Python suite, and all
...
`docker-compose.yml` and put a reverse proxy (Caddy/nginx) with TLS in front â€”
plus complete the security checklist (rate limiting, CORS origins, monitoring)
from PROJECT_HANDOFF Â§51 before going public.

## CI

`.github/workflows/ci.yml` runs two jobs on push/PR:

1. **python** â€” Python 3.13, `pip install -e ".[dev]"`, `ruff check .`,
   `python -m pytest -q`.
2. **web** â€” Node 22, `npm ci`, then lint, typecheck, unit tests, and build.
